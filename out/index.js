...
function parseInputs() {
    var inputs = Object.keys(process.env)
        .filter(function (key) { return key.startsWith(INPUT_PREFIX); })
        .filter(function (key) { return process.env[key] && !DEFAULT_INPUTS.includes(key); })
        .map(function (key) {
        return {
            id: key,
...
function getElseIfs(inputs) {
    ...
    var elseIfs = inputs
        .filter(function (i) {
        return getCondition(i.id).startsWith(ELSE_IF) &&
    ...
    for (var _i = 0, elseIfs_1 = elseIfs; _i < elseIfs_1.length; _i++) {
        var elseIf = elseIfs_1[_i];
        ...
        var elseIfId = elseIf.id.split(ELSE_IF)[1];
        conditionals.push({
...
function logResult(condition, outputs) {
    var outs = outputs.map(function (o) { return o.id; }).join(", ");
    var cond = condition.replace(INPUT_PREFIX, "");
    if (condition === ELSE) {
        core_1.info("Else block hit.  Set outputs(s) [" + outs + "]");
    }
    else {
        core_1.info(cond + " evaluated to true.  Set output(s) [" + outs + "]");
    }
}
function run() {
    var _a;
...
run().catch(function (e) {
    core_1.error("An error occurred: " + e);
    process.exit(1);
});
